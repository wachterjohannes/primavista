...
Nice to have, not used by Sulu today: paste from Word cleanup, autoformat shortcuts, word count. Autoformat and word count exist now as opt-in plugins (`suluPlugins({ autoformat: true })`, `wordCount()`), see decision 35 in `DECISIONS.md`.
