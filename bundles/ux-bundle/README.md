...
Everything else is dropped: scripts, event handlers, `javascript:` URLs, other inline styles, classes, unknown elements with their content. The sanitizer writes its own serialization (`<br />`, entity-encoded characters in attributes), which the editor reads back unchanged.
...
The defaults apply while FrameworkBundle's `html_sanitizer` is enabled, which it is as soon as `symfony/html-sanitizer` is installed. The bundle requires it. The same sanitizer is available as the Twig filter `sanitize_html('primavista')` and for autowiring as `HtmlSanitizerInterface $primavistaSanitizer`.
