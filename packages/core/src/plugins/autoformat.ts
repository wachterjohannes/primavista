...
import { type Allowed, collectAllowed } from './allowed';
...
const ALL_LEVELS: ReadonlyArray<HeadingTagType> = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
 ...
 * other plugins offer is converted, so the shortcuts follow `headings`,
 * `lists` and `formatting` and their options. It changes the document the
 * way a toolbar button would and never touches HTML import or export.
 */
export function autoformat(options: AutoformatOptions = {}): PrimavistaPlugin {
  return {
    name: 'autoformat',
    register: ({ editor, plugins }) => {
      const transformers = autoformatTransformers(editor, collectAllowed(plugins), options);
      return transformers.length > 0 ? registerMarkdownShortcuts(editor, transformers) : undefined;
    },
  };
}

function autoformatTransformers(editor: LexicalEditor, allowed: Allowed, options: AutoformatOptions): Transformer[] {
  const transformers: Transformer[] = [];
  if (options.blocks ?? true) {
    if (editor.hasNodes([HeadingNode])) {
      transformers.push(restrictHeadings(allowed.headings ?? ALL_LEVELS));
    }
    if (editor.hasNodes([ListNode, ListItemNode])) {
      const types = allowed.lists ?? ['ul', 'ol'];
      if (types.includes('ul')) transformers.push(UNORDERED_LIST);
      if (types.includes('ol')) transformers.push(ORDERED_LIST);
    }
  }
