...
export interface PasteCleanupOptions {
  /** Inline formats pasted content may keep. Pass the same list as to `formatting()`. Defaults to all. */
  formats?: ReadonlyArray<InlineFormat>;
  /** Keep `text-align` from pasted content. Pass false without `alignment()`. Defaults to true. */
  alignment?: boolean;
}

/**
 * Cleans pasted HTML from Word, Google Docs, LibreOffice and web pages with
 * `cleanPastedHtml` before Lexical imports it. Headings, lists, links, tables
 * and language spans survive when their plugin is registered. Plain text and
 * content copied inside Primavista take Lexical's usual path.
 */
export function pasteCleanup(options: PasteCleanupOptions = {}): PrimavistaPlugin {
  return {
    name: 'paste-cleanup',
    register: ({ editor }) => {
      const schema: CleanPastedHtmlOptions = {
        headings: editor.hasNodes([HeadingNode]),
        lists: editor.hasNodes([ListNode]),
        links: editor.hasNodes([LinkNode]),
        tables: editor.hasNodes([TableNode]),
        language: editor.hasNodes([LanguageNode]),
        alignment: options.alignment ?? true,
      };
      if (options.formats) schema.formats = options.formats;
